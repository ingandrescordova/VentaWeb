<?php
	...
	class NeuralBDConsultas {
		...
		**/
		...
		/**
		 * AgregarInnerJoin($Tabla)
		 * 
		 * Agregamos la tabla a consultar por Inner Join
		 * */
		public function AgregarInnerJoin($Tabla) {
			$this->TablasInnerJoin[] = trim($Tabla);
		}
		
		/**
		 * AgregarCondicionInnerJoin($Condicion)
		 * 
		 * Agregamos la condicion de consulta Inner Join
		 * */
		public function AgregarCondicionInnerJoin($Condicion) {
			$this->CondicionInnerJoin[] = trim($Condicion);
		}
		...
		public function PrepararQuery() {
			...
			$InnerJoin='';
			if(isset($this->TablasInnerJoin) AND isset($this->CondicionInnerJoin)){
				$Limite = count($this->TablasInnerJoin);
				for($i = 0; $Limite > $i; $i++) 
					$InnerJoin .= ' INNER JOIN '.$this->TablasInnerJoin[$i].' ON '.$this->CondicionInnerJoin[$i];	
			}
			else {
				$InnerJoin = NULL;
			}
			...
			if($TablasOrganizadas!=NULL AND $ColumnasOrganizadas!=NULL) {
				...
				//Validamos si tenemos los parametros para inner join
				if($InnerJoin!=NULL)
					$ConstructorBase .= $InnerJoin;
				...
				unset($TablasOrganizadas, $ColumnasOrganizadas, $CondicionesOrganizadas, $AgruparOrganizadas, $OrdenarOrganizadas, $LimiteOrganizadas, $InnerJoin, $Limite);
