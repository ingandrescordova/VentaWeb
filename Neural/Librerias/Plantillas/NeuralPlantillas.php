<?php
	...
	class NeuralPlantillasTwig {
		...
		/**
		 * Metodo Publico
		 * AgregarFuncionParametros($NombreFuncion = false, $Funcion = false)
		 * 
		 * Metodo en el cual se pueden pasar funciones con parametros a la plantilla
		 * @param $NombreFuncion: Nombre que tendra la funcion
		 * @Funcion $Funcion: Funcion que se ejecutara
		 */
		public function AgregarFuncionParametros($NombreFuncion = false, $Funcion = false) {
			if($NombreFuncion == true AND $Funcion == true) {
				$this->FuncionParametros[$NombreFuncion] = $Funcion;
			}
		}
			...
			if($Plantilla == true) {
				...
				if(file_exists(__SysNeuralFileRootModulos__.$CarpetaAplicacion['Carpeta'].$CarpetaAplicacion['Vistas'].$Plantilla)) {
					...
					if($Cache == true) {
						...
						if(isset($this->FuncionParametros) == true) {
							foreach ($this->FuncionParametros AS $Nombre => $Funcion) {
								$Filtro = new Twig_SimpleFunction($Nombre, $Funcion);
								$Twig->addFunction($Filtro);
							}
						}
